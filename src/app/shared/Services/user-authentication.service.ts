...
export class UserAuthenticationService {
  private userRoles: UserDetail[] = []; // List of roles for the current user

  constructor(private _router:Router, private _localStorageManagerService: LocalStorageManagerService) {  
    this.userRoles = this._localStorageManagerService.getUserDetail();
  }
  ...
  hasRole(role: string): boolean {
    return this.userRoles.some((userRole) =>
      userRole.rolePermission?.name?.includes(role)
    );
  }
}
